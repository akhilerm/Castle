...
function error_handler($err, $message, $file, $line) {
    ...
    $error = array(
        ...
        "error" => array(
            ...
            "line" => $content[$line-1]));
    ob_end_clean();
    echo responseRpc(null, $id, $error);
    exit();
}
// ----------------------------------------------------------------------------
...
function responseRpc($result, $id, $error) {
    if ($error) {
    ...
    try {
        ...
        if (strcmp($method, "system.describe") == 0) {
            ...
            if (in_array("__call", $methods) && !in_array($method, $methods)) {
                ...
                echo responseRpc($result, $id, null);
            } else {
        ...
        echo responseRpc(null, $id, array("code"=>$code, "message"=>$msg));
    } catch (Exception $e) {
        //catch all exeption from user code
        $msg = $e->getMessage();
        echo responseRpc(null, $id, array("code"=>200, "message"=>$msg));
    }
    ob_end_flush();
}